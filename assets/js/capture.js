(function () {
  ...
  let data_array = [];
  let data_array_absorbtion = [];
  let data_array_peak = [];

  let minIntensityAbsorption = 10;
  ...
  function myLineTo(ctx, x1, y1, x2, y2, minY, maxY) {
    if (y1 <= maxY && y2 <= maxY && y1 >= minY && y2 >= minY) {
      // Both points are within the minY and maxY thresholds, so draw the entire line
      ctx.lineTo(x2, y2);
    } else if (y1 > maxY && y2 > maxY || y1 < minY && y2 < minY) {
      // Both points are either above the maxY threshold or below the minY threshold, so don't draw anything
    } else {
      // Calculate the intersection points with the minY and maxY thresholds
      const slope = (y2 - y1) / (x2 - x1);
      let xInterceptMaxY = x1 + (maxY - y1) / slope;
      let yInterceptMaxY = maxY;
      let xInterceptMinY = x1 + (minY - y1) / slope;
      let yInterceptMinY = minY;

      // Check which intersection points are within the bounds of the line segment
      if (y2 >= minY && y2 <= maxY) {
        if (y1 > maxY) {
          ctx.moveTo(x2, y2);
          ctx.lineTo(xInterceptMaxY, yInterceptMaxY);

        }
        else {
          ctx.moveTo(x2, y2);
          ctx.lineTo(xInterceptMinY, yInterceptMinY);
        }
      ...
      // Draw the line segment between the intersection points
  ...
  function setElements() {
    ...
    var s = document.getElementById('switch-peaks');
    s.checked = showPeaks;

    var s = document.getElementById('ifactor');
    s.value = ifactor;

    var s = document.getElementById('switch-exposureMode');
    s.checked = manualExposure;
    if (manualExposure)
    document.getElementById("exposureTime").style.display = "";
     else
     document.getElementById("exposureTime").style.display = "none";
  ...
  function getCookies() {
    ...
    if (getCookie("peaks") != "") { if (getCookie("peaks") == 1) showPeaks = true; else showPeaks = false; }
    if (getCookie("ifactor") != "") ifactor = parseFloat(getCookie("ifactor"));
    if (getCookie("manualExposure") != "") { if (getCookie("manualExposure") == 1) manualExposure = true; else manualExposure = false; }
  ...
  function setCookies() {
    ...
    setCookie("peaks", showPeaks * 1, 10);
    setCookie("ifactor", ifactor, 10);
    setCookie("manualExposure", manualExposure * 1, 10);
  ...
  function setVideoSettings() {
    ...
    video = document.getElementById('video');
        ...
        console.log('Supported video capabilities:');
        ...
        // settings.exposureTime=2000;
        // const Supconstrains = track.getConstraints();
        // console.log('Constraint:');
        // console.log(Supconstrains);
  ...
  async function saveFileAs(myArray, fileName) {
    ...
    for (let i = 0; i < myArray.length / 4; i++) {
  ...
  function startup() {
    ...
    video.addEventListener('canplay', function (ev) {
      if (!streaming) {
        ...
        if (manualExposure) {
          constraintsToSet = { advanced: [{ exposureMode: "manual" }] };
        } else {
          constraintsToSet = { advanced: [{ exposureMode: "continuous" }] };
        }
        const track = video.srcObject.getVideoTracks()[0];
        ...
        document.getElementById('switch-exposureMode').addEventListener('change', function (e) {
          let constraintsToSet = {};
          if (e.target.checked) {
            constraintsToSet = { advanced: [{ exposureMode: "manual" }] };
            document.getElementById("exposureTime").style.display = "";
            manualExposure=true;
        ...
        document.getElementById('exposureTime').addEventListener('input', function (e) {
          ...
          let constraintsToSet = { advanced: [{ exposureTime: value }] };
        ...
        document.getElementById('switch-peaks').addEventListener('change', function (e) {
        ...
        document.getElementById('switch-absorbtion').addEventListener('change', function (e) {
          ...
          if (e.target.checked) {
            ...
            for (let i = 0; i <= data_array.length; i++) {
              data_array_absorbtion[i] = data_array[i];
            }
    ...
    playbutton.addEventListener('click', function () {
      ...
      if (isrunning) isrunning = false; else isrunning = true;
    ...
    exportbutton.addEventListener('click', function () {
      ...
      saveFileAs(data_array, "spectrum.csv")
  ...
  function takepicture() {
    ...
    if (width && height) {
      ...
      if (isrunning) {
        ...
        // temporal filtering for peak finding      	        
        for (let i = 0; i <= data.data.length; i++) {


          if (isNaN(data_array_peak[i])) data_buffer[i] = data.data[i];
          else data_buffer[i] = data_array_peak[i];
        }

        for (let i = 0; i <= data.data.length; i++) {
          data_array_peak[i] = (0.9) * data_buffer[i] + (1 - 0.9) * data.data[i];
        }
      ...
      if (color_mode) {
        ...
        for (let i = imin + 1; i <= imax; i++) {
          data_value_last = data_value;
          data_value = data_array[i * 4];

          myLineTo(ctx, transX(pixToWave(i - 1), mySettings), transY(data_value_last, mySettings), transX(pixToWave(i), mySettings), transY(data_value, mySettings), transY(mySettings.maxY, mySettings), transY(mySettings.minY, mySettings));
        ...
        for (let i = imin + 1; i <= imax; i++) {
          ...
          if (absorptionMode) {
            let absorbtion_value = (data_array_absorbtion[i * 4] + data_array_absorbtion[i * 4 + 1] + data_array_absorbtion[i * 4 + 2]);
            if (absorbtion_value > minIntensityAbsorption) data_value = 100 * Math.log(absorbtion_value / data_value);
            else data_value = 0;
          }
          if ((data.data[i * 4] > clipping_limit) || (data.data[i * 4 + 1] > clipping_limit) || (data.data[i * 4 + 2] > clipping_limit)) clipping = true;
        ...
        if (showPeaks) {
          let peaks_array = [];
          for (let i = 0; i < data_array_peak.length / 4; i++) {
            peaks_array[i] = (data_array_peak[i * 4] + data_array_peak[i * 4 + 1] + data_array_peak[i * 4 + 2])
          }

          let peaks = peakDetection(peaks_array, 30);

          ctx.strokeStyle = '#f35b3f';  // set color to gray
          ctx.lineWidth = 1;
          ctx.font = "10px Arial";
          ctx.textAlign = "left";
          ...
          for (let i = 0; i < peaks.length; i++) {
            ...
            if ((pixToWave(peaks[i]) > mySettings.minX) && (pixToWave(peaks[i]) < mySettings.maxX)) {
              data_value = (data_array[peaks[i] * 4] + data_array[peaks[i] * 4 + 1] + data_array[peaks[i] * 4 + 2]);

              ctx.moveTo(transX(pixToWave(peaks[i]), mySettings), transY(data_value, mySettings) - 5);
              ctx.lineTo(transX(pixToWave(peaks[i]), mySettings), transY(data_value, mySettings) - 20);


              ctx.fillText(pixToWave(peaks[i]).toFixed(1), transX(pixToWave(peaks[i]), mySettings) + 5, transY(data_value, mySettings) - 20);
            }
